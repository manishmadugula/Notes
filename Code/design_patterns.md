...
- Especially useful if the instantiation is complex.
- Polymorphism, allows us to swap one factory to another factory.
- Holds the business logic for creation of similar subtypes, say there is a RandomAnimalFactory creates an animal of Dog or Cat or Duck randomly. 
- Defines an API for creating an object.



## Simple Factory
- No polymorphism, creating Concrete Products from Concrete Factories probably a single factory.

## Factory Method
![](res/factory-design-pattern.jpeg)
- One more application is that game designing where the asteriods and powerup are autogenerated based on Level, we can pass the level parameter to the factory Method and create factories (Asteroid Factory and Powerup Factory) using level logic and randomness to create asteroids . Thus we move away from class explosion
## Abstract Factory
- Factory that creates multiple products of same or different types.
- Lets you create families of related or dependent products. Say in our application it only makes sense to use specific pair of products together, eg : UI elements. Mac button needs to go with Mac alert box. 
- We can use a factory which creates 2 products. Say our AbstractFactory can be MacOSFactory, WindowsFactory, LinuxFactory, call macFactory.getButton() will give Mac specific button and macFactory.getAlert() will give Mac specific Alert.

![](res/abstract_factory_method.jpeg)
